...
}}function showAllColorsModal(){$('#allColorsModal').remove();let modalHtml=`<div class="modal fade" id="allColorsModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered modal-lg"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">All Colors</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body"><div class="color-grid">`;colors.forEach((color,index)=>{modalHtml+=`<div class="color-grid-item" data-index="${index}"><div class="color-image"><img src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 160 160'%3E%3C/svg%3E" data-src="${color.path}" width="160" height="160" loading="lazy" class="lazyload" alt="${color.name}" onerror="this.onerror=null; this.src='images/placeholder.jpg';"></div><div class="color-name">${color.name}</div></div>`;});modalHtml+=`</div></div></div></div></div>`;$('body').append(modalHtml);const modal=new bootstrap.Modal(document.getElementById('allColorsModal'));modal.show();setTimeout(initLazyLoading, 100);}function showColorFullscreen(index){currentColorIndex=index;const color=colors[index];$('.color-fullscreen-container').remove();const fullscreenHtml=`<div class="color-fullscreen-container"><div class="color-fullscreen-content"><img src="${color.path}" alt="${color.name}" class="color-fullscreen-image" onerror="this.onerror=null; this.src='images/placeholder.jpg';"><div class="color-fullscreen-caption">${color.name}</div><button class="color-fullscreen-nav color-fullscreen-prev"><i class="bi bi-chevron-left"></i></button><button class="color-fullscreen-nav color-fullscreen-next"><i class="bi bi-chevron-right"></i></button><button class="color-fullscreen-close"><i class="bi bi-x"></i></button></div></div>`;$('body').append(fullscreenHtml);$('body').addClass('overflow-hidden');const modal=bootstrap.Modal.getInstance(document.getElementById('allColorsModal'));if(modal){modal.hide();}}function closeColorFullscreen(){$('.color-fullscreen-container').remove();$('body').removeClass('overflow-hidden');if($('#allColorsModal').length){const modal=new bootstrap.Modal(document.getElementById('allColorsModal'));modal.show();}}function navigateColors(direction){if(direction==='prev'){currentColorIndex=(currentColorIndex-1+colors.length)%colors.length;}else{currentColorIndex=(currentColorIndex+1)%colors.length;}const color=colors[currentColorIndex];$('.color-fullscreen-image').attr('src',color.path).attr('alt',color.name).on('error',function(){$(this).attr('src','images/placeholder.jpg');});$('.color-fullscreen-caption').text(color.name);}$(window).on('load',function(){setTimeout(initColorCarousel,300);});})(jQuery);
