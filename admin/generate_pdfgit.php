...
if (isset($_GET['id'])) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        foreach ($items_data as $item) {
            ...
            if ($type === 'marker') {
                $height = 3; // Standard height for markers
            } elseif ($type === 'slant') {
            ...
            // Calculate base price with color adjustment
            $base_price = $item['base_unit_price'];
            
            // Add commission to get final unit price
            $unit_price = round($base_price * (1 + ($row['commission_rate'] / 100)), 2);
...
foreach ($data['items'] as $item) {
    ...
    if ($item['type'] === 'marker') {
        $dimensions = sprintf('%.2f" × %.2f" × 3.00"', $item['length'], $item['breadth']);
        $cubic_feet = round(($item['length'] * $item['breadth'] * 3 * $item['quantity']) / 1728, 2);
    } elseif ($item['type'] === 'slant') {
        $dimensions = sprintf('%.2f" × %.2f" × 6.00"', $item['length'], $item['breadth']);
        $cubic_feet = round(($item['length'] * $item['breadth'] * 6 * $item['quantity']) / 1728, 2);
    } else {
        $dimensions = sprintf('%.2f" × %.2f" × %.2f"', $item['length'], $item['breadth'], $item['size']);
