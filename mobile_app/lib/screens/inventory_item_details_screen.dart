...
class _InventoryItemDetailsScreenState
  ...
  Future<void> _loadDetailedInfo() async {
    ...
    final records =
        await inventoryService.getItemDetailedRecords(widget.item.code);
  ...
  void _checkIfItemIsSaved() {
    final savedItemsState =
        Provider.of<SavedItemsState>(context, listen: false);
    setState(() {
  ...
  Future<void> _toggleSaveItem() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content:
              Text(isSaved ? 'Item saved!' : 'Item removed from saved items'),
  ...
  void _addToCart() {
    ...
    final cartItem = {
      'id':
          widget.item.code, // 'id' is used for identifying unique items in cart
      'code': widget.item.code,
      'description': widget.item.description,
      'quantity': quantity,
      'color': widget.item.color,
    ...
    final existingIndex =
        cartState.items.indexWhere((item) => item['id'] == cartItem['id']);

    if (existingIndex >= 0) {
      // Update existing item quantity
      final currentQuantity = cartState.items[existingIndex]['quantity'] as int;
      cartState.updateQuantity(
          cartState.items[existingIndex], currentQuantity + quantity);
    } else {
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow(
                context,
                'Length',
                widget.item.lengthInInches.isNotEmpty
                    ? widget.item.lengthInInches
                    : 'Not specified'),
            _buildInfoRow(
                context,
                'Height',
                widget.item.heightInInches.isNotEmpty
                    ? widget.item.heightInInches
                    : 'Not specified'),
            _buildInfoRow(
                context,
                'Width',
                widget.item.widthInInches.isNotEmpty
                    ? widget.item.widthInInches
                    : 'Not specified'),
            _buildInfoRow(
                context,
                'Weight',
                widget.item.weight.isNotEmpty
                    ? '${widget.item.weight} lbs'
                    : stoneRecords.isNotEmpty &&
                            stoneRecords[0].weight.isNotEmpty
                        ? '${stoneRecords[0].weight} lbs'
                        : 'Not specified'),
            ...
            const SizedBox(height: 8),
            _buildInfoRow(
                context, 'Total Available', widget.item.quantity.toString()),
            _buildInfoRow(context, 'Location', widget.item.location),
            _buildInfoRow(
                context,
                'Status',
                stoneRecords.isNotEmpty && stoneRecords[0].status.isNotEmpty
                    ? stoneRecords[0].status
                    : 'In-Stock'),
            if (stoneRecords.isNotEmpty &&
                stoneRecords[0].sublocation.isNotEmpty)
              _buildInfoRow(
                  context, 'Sublocation', stoneRecords[0].sublocation),

            // Show individual stone records with Container and Crate info
            if (stoneRecords.isNotEmpty) ...[
              const SizedBox(height: 24),
              _buildSectionHeader(
                  context, 'Individual Stones (${stoneRecords.length})'),
              const SizedBox(height: 8),
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (stone.stockId > 0)
                          _buildInfoRow(
                              context, 'Stock ID', stone.stockId.toString()),
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Padding(
                        ...
                        child: Semantics(
                          button: true,
                          label: 'Open product image ${index + 1}',
