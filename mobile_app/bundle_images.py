...
def get_product_categories() -> List[Dict]:
    ...
    url = f"{API_ENDPOINT}?directory=products"
    data = fetch_json(url)
    
    if not data.get('success'):
        raise Exception(f"API error: {data.get('error', 'Unknown error')}")
    
    categories = data.get('files', [])
    print(f"✅ Found {len(categories)} categories")
    return categories

def get_category_images(category_name: str) -> List[Dict]:
    """Fetch all images for a specific category"""
    encoded_name = urllib.parse.quote(category_name)
    url = f"{API_ENDPOINT}?directory=products/{encoded_name}"
    data = fetch_json(url)
    
    if not data.get('success'):
        print(f"⚠️  Warning: Could not fetch images for {category_name}: {data.get('error')}")
        return []
...
def download_category_images(category_name: str) -> Dict:
    ...
    for idx, img in enumerate(images, 1):
        ...
        dest_path = category_dir / safe_filename_noext
...
def main():
    ...
    try:
        ...
        print("⚠️  NOTE: This will increase app size but images are optimized")
